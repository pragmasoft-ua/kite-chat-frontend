...
export class HumaneChat extends LitElement {
  ...
  readonly chatId?: string = randomStringId();
  ...
  private _send() {
    if (this.textarea.value?.length > 0) {
      ...
      this.insertAdjacentHTML(
        'beforeend',
        `<humane-msg status="${status}" msgId="${msgId}" datetime="${datetime.toISOString()}">${msg}</humane-msg>`
      );
      ...
      this.dispatchEvent(
        new CustomEvent('send', {
          detail: {chatId: this.chatId, msgId, status, datetime, msg},
        })
      );
    }
  }
  ...
  incoming(msg: String, msgId = randomStringId(), datetime = new Date()) {
    console.debug('receive', msg, msgId, datetime);
    this.insertAdjacentHTML(
      'beforeend',
      `<humane-msg msgId="${msgId}" >${msg}</humane-msg>`
    );
    this.lastElementChild?.scrollIntoView();
  }
