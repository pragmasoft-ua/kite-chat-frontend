...
export class KiteChatElement extends LitElement {
  ...
  private _edit() {
    //TODO api call
    const editedMsg = [...this.messageElements].find((msgElement) => (
      this.selectedMessages.includes(msgElement.messageId)
    ));
    if (editedMsg) {
      if(!editedMsg.querySelector('kite-file')) {
        this.textarea.value = editedMsg.innerText;
        this.textarea.focus();
      } else {
  ...
  editMsg(msg: KiteMsg) {
    ...
    const editedMsg = [...this.messageElements].find((msgElement) => (
      msgElement.messageId === msg.messageId
    ));
    if(!editedMsg) {
      return;
    }
    const {messageId, timestamp = new Date(), status} = msg;
    editedMsg.messageId = messageId;
    editedMsg.timestamp = timestamp;
    editedMsg.status = status;
    editedMsg.edited = true;
    if (isPlaintextMsg(msg)) {
      editedMsg.innerText = msg.text;
    } else {
      const {file} = msg;
      const fileElement = editedMsg.querySelector('kite-file');
      fileElement && (fileElement.file = file);
    }
    editedMsg.unselect();
    editedMsg.scrollIntoView(false);
  }

  private _sendText() {
    if (this.textarea.value?.length > 0) {
      const editMsg = this.selectedMessages.length === 1;
      const message: PlaintextMsg = {
        messageId: editMsg ? this.selectedMessages[0] : randomStringId(),
        timestamp: new Date(),
